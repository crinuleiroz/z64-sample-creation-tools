...
class FileProcessor:
  ...
  def convert_predictors(codebook: str, *loopbook: str) -> None:
    predictor_files = [(codebook, 'codebook')] + [(loopbook[0], 'loopbook')] if loopbook else [(codebook, 'codebook')]
  ...
  def move_processed_files(self, outfolder: str, filename: str, zsound: str) -> None:
    ...
    #os.rename('config.toml', os.path.join(outfolder, f'{filename}_config.toml'))

  def move_bank_xml(self, outfolder: str, filename: str) -> None:
    os.rename(f'{filename}_BANK.xml', os.path.join(outfolder, f'{filename}_BANK.xml'))
  ...
  def process_all_files(self) -> None:
    ...
    for file in self.files:
      if os.path.isdir(file):
        # Recursive searching through directory
...
class AudioProcessor:
  ...
  def extract_wave_data(self) -> tuple[int, int]:
    ...
    try:
      ...
      raise RuntimeError(f'An error occurred while extracting WAV data: {e}')

    wav.parse_wave()

    # Ensure Polyphone did not ruin the sample before sending it to z64audio
    if wav.smpl_chunk:
      if wav.smpl_chunk.loops[0].loop_start == 0x00000000 \
        and wav.smpl_chunk.loops[0].loop_end == 0xFFFFFFFF:
        wav.fix_loop()
        wav.save()
  ...
  def run_z64audio(self, filename : str) -> None:
    ...
    try:
      subprocess.run(
        ['z64audio', '-i', self.filepath, '-o', f'{filename}.bin', '-I', '50'],
        check=True,
