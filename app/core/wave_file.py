...
class SmplChunk(ctypes.Structure):
    ...
    def __init__(self, manufacturer=b'', product=b'', sample_period=0, midi_unity_note=0, midi_pitch_fraction=0, smpte_format=0, smpte_delay=0, num_loops=0, sample_data=0, loops=None):
        ...
        # This will generally just be a single array
        self.loops = (LoopStruct * len(loops))(*loops)
...
class WaveFile:
    ...
    def read_header(self):
        ...
        if chunk_id != b'RIFF' or format != b'WAVE':
            raise ValueError(
                f'Invalid WAV file. Expected RIFF and WAVE format got: {chunk_id} and {format}.')
    ...
    def parse_fmt(self):
        ...
        if len(fmt_data) < 16:
            raise ValueError('Size of fmt chunk is too small!')

        (
            self.fmt_chunk.audio_format,
            self.fmt_chunk.num_channels,
            self.fmt_chunk.sample_rate,
            self.fmt_chunk.byte_rate,
        ...
        if self.fmt_chunk.audio_format != 1:
            raise ValueError(
                f'Unsupported WAV format {self.fmt_chunk.audio_format} '
                'only PCM is supported.'
                'Use FFMPEG to convert to PCM: ffmpeg -i <in> -acodec pcm_s16le <out>'
            )

        if self.fmt_chunk.num_channels == 0:
            raise ValueError('WAV fmt chunk has 0 channels!')

        if self.fmt_chunk.bits_per_sample == 0:
            raise ValueError('WAV fmt chunk has 0 bits per sample!')

        if self.fmt_chunk.audio_format != 1:
            extra = fmt_data[16:]
            buf = ctypes.create_string_buffer(extra)
            self._fmt_extra_buf = buf
            self.fmt_chunk.extra_params = ctypes.cast(buf, ctypes.c_void_p)
            # self.fmt_chunk.extra_params = fmt_data[16:]

    def find_smpl_chunk(self):
        self.smpl_chunk_addr, self.smpl_chunk_size = self._scan_for_chunk(
            b'smpl')
    ...
    def parse_smpl(self):
        ...
        (
            self.smpl_chunk.manufacturer,
            self.smpl_chunk.product,
        ...
        for _ in range(self.smpl_chunk.num_loops):
            loop_data = struct.unpack(
                '<6I', smpl_data[loop_offset:loop_offset+24])
    ...
    def find_data_chunk(self):
        self.data_chunk_addr, self.data_chunk_size = self._scan_for_chunk(
            b'data')
